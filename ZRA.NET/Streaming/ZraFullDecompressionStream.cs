...
namespace ZRA.NET.Streaming
{
    public class ZraFullDecompressionStream : Stream
    {
         ...
         */
        public unsafe ZraFullDecompressionStream(Stream baseStream, bool leaveOpen = false)
        {
            ...
            ZraHeader = LibZra.ZraGetHeaderWithFullDecompressor(_fullDecompressor);
         ...
         * Reads a sequence of bytes from the current stream and decompresses it, then advances the position within the stream by the number of bytes read.
         * </summary>
         * <param name="buffer">An span of bytes. When this method returns, the buffer contains the bytes decompressed from the current source.</param>
         * <returns>The total number of bytes read and decompressed into the buffer.
         * This can be less than the number of bytes requested if that many bytes are not currently available,
         * or zero (0) if the end of the stream has been reached.</returns>
         */
        ...
        public unsafe int Read(Span<byte> buffer, int offset, int count)
        {
            if (offset != 0)
            {
                throw new ArgumentOutOfRangeException(nameof(offset), "This stream is non-seekable, therefore passing a value to the offset parameter is not allowed.");
            }

            fixed (byte* bufferPtr = buffer)
            {
                LibZra.ZraDecompressWithFullDecompressor(_fullDecompressor, bufferPtr, (ulong)count, out ulong outputSize).ThrowIfError();
        ...
        {
            ulong bufferSize = LibZra.ZraGetFrameSizeWithHeader(ZraHeader);
